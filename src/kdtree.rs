...
pub struct KdNode<const N: usize> {
    pub point: [f32; N],
    pub left: Option<Box<KdNode<N>>>,
    pub right: Option<Box<KdNode<N>>>,
    pub level: usize,
}
...
pub fn partition<const N: usize>(
    ...
    if points.len() == 2 {
        return (points[1], &mut points[0..1], Default::default());
    }
    let m = points.len() / 2;
...
mod tests {
    ...
    #[test]
    fn test_partition2() {
        let mut points = [[1.0, -1.0], [-1.0, 6.0], [0.0, 5.0]];
    ...
    #[test]
    fn test_partition3() {
        let mut points = [[1.0, -1.0], [-1.0, 6.0]];
